...
def get_weather_data():
    ...
    try:
        logger.info(f"Fetching weather data from OpenWeatherMap API for Moscow (lat: {LAT}, lon: {LON})")
        ...
        ) if data.get('dt') else timezone.now()

        sunrise_time = timezone.make_aware(
            datetime.fromtimestamp(sys_data.get('sunrise', 0))
        ) if sys_data.get('sunrise') else None

        sunset_time = timezone.make_aware(
            datetime.fromtimestamp(sys_data.get('sunset', 0))
